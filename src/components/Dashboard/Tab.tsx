...
const TabNavigation: React.FC<TabNavigationProps> = ({ children }) => {
  ...
  return (
    <div>
      ...
      <div className="p-4 bg-gray-100">
        {children.map((child, index) => (
          <div
            key={tabs[index].id}
            className={
              activeTab === tabs[index].id ? "block overflow-y-auto" : "hidden"
            }
          >
            {child}
          </div>
      ...
      <div className="p-4 bg-white border-t-2 border-gray-200 flex flex-row justify-center items-center">
        <p>بستن</p>
      </div>
    </div>
  );
