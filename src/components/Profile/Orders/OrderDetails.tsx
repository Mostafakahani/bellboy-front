...
export default function OrderDetails({ order, onClose }: OrderDetailsProps) {
  ...
  const isDiscounted = order.totalPayable < order.totalPrice + order.tax + order.shippingCost;
  const isPending = order.status === "در حال تامین";

  return (
    <div className="relative">
      <div className="flex justify-between items-center p-6">
        <h1 className="text-xl font-bold">سفارش {order.id}</h1>
      ...
      <div className="flex flex-col gap-4 px-6 pb-5 border-b-2 border-black">
        ...
        <div className="flex justify-between">
          ...
          <span className="text-sm">{formatCurrency(order.totalPayable)}</span>
        </div>
      </div>
    </div>
  );
